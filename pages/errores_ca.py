...
FROM parkimovil-app.geosek_raspis.log_sek LS
...
proyectos_ca = client.query(ca_proyectos).to_dataframe()

errores_ca = client.query(errores_ca).to_dataframe()

list_proyectos_ca = proyectos_ca['proyecto'].unique().tolist()
list_errores_ca = errores_ca['tipo_error'].unique().tolist()
list_errores_ca = list(set(error.strip() for error in list_errores_ca))
...
def proyectos_errores(proyecto, error, intervalo_tiempo):
    ...
    count_error = f"""
        ...
        JOIN parkimovil-app.geosek_raspis.raspis R
            ON LS.QR = R.qr
        WHERE EXTRACT(DATE FROM TIMESTAMP_ADD(date, INTERVAL -6 HOUR)) >= '2024-01-01' 
            AND TRIM(R.alias) LIKE '{proyecto}' 
            AND TRIM(LS.function_) LIKE '{error}'
        GROUP BY {group_by_clause}
        ORDER BY fecha ASC;
    """
    df_proyectos_errores_ca = client.query(count_error).to_dataframe()
    return df_proyectos_errores_ca

def tipo_errores_ca(proyecto):
    errores_proyecto = f"""
    SELECT LS.function_ AS tipo, COUNT(function_) AS cantidad
    FROM parkimovil-app.geosek_raspis.log_sek LS
    JOIN parkimovil-app.geosek_raspis.raspis R
        ON LS.QR = R.qr
    WHERE TRIM(R.alias) LIKE '{proyecto}' AND EXTRACT(DATE FROM TIMESTAMP_ADD(date, INTERVAL -6 HOUR)) = CURRENT_DATE('America/Mexico_City')
    GROUP BY LS.function_
    """

    df_tipo_errores_ca = client.query(errores_proyecto).to_dataframe()
    return df_tipo_errores_ca
...
def errores_hora(proyecto):
    ...
    FROM
    parkimovil-app.geosek_raspis.log_sek LS
    JOIN
...
proyecto_seleccionada = st.selectbox('Selecciona una ciudad:', list_proyectos_ca)
error_seleccionado = st.selectbox('Selecciona un tipo de error:', list_errores_ca)
...
df_tipo_errores_proyecto = tipo_errores_ca(proyecto_seleccionada)
...
fig.add_trace(
    go.Scatter(
        ...
        mode='lines+markers',
        name='Lectura',  # Name for the legend
...
# Crear una tabla con los mismos datos
...
# Añadir el gráfico de pastel y la tabla
fig_tabla.add_trace(tipo_fig.data[0], row=1, col=1)
fig_tabla.add_trace(tabla_datos, row=1, col=2)

# Actualizar diseño y mostrar
fig_tabla.update_layout(title_text=f'Operaciones - {fecha_actual}',
                  height=600)
